...
%
%----------------------------------------20211020
% BallberrySlope (m) %[] medlyn coefficient 'm' fot stomatal regulation
% Ballberry0  % [] (OPTIONAL) Medlyn intercept term 'b' (if present, an iterative solution is used)
% ----------------------------------------------------------------------------
...
% VPD_l2b = equations.satvap(T-273.15).*exp(1E-3*PSI*MH2O/R./(T))-eb;
...
elseif all(BallBerry0 == 0)
    ...
    Ci = BallBerry(Cs, A, VPD_l2b, BallBerrySlope, BallBerry0, minCi);
    A =  computeA(Ci);
...
%    (if iteration was used). In other words, A is assigned at this point in the file (when iterating).
    function [err, Ci_out] = Ci_next(Ci_in, Cs, RH, minCi)
        ...
        A = computeA(Ci_in);
        A_bar = A .* ppm2bar;
        Ci_out = BallBerry(Cs, A_bar, VPD_l2b, BallBerrySlope, BallBerry0, minCi); %[Ci_out, gs]
...
% end
%% Medlyn gs
% function [Ci, gs] = BallBerry(Cs, A, VPD_l2b, BallBerrySlope, BallBerry0, minCi, Ci_input)
% %  Cs  : CO2 at leaf surface					  
% %  A   : Net assimilation in 'same units of CO2 as Cs'/m2/s
...
%     gs = [];
%     if ~isempty(A) && nargout > 1
%         gs = gsFun(Cs,  A, VPD_l2b, BallBerrySlope, BallBerry0);
%     end
% elseif all(BallBerry0 == 0) || isempty(A)
%     % EXPLANATION:   *at equilibrium* CO2_in = CO2_out => A = gs(Cs - Ci) [1]
%     %  so Ci = Cs - A/gs (at equilibrium)                                 [2]
...
%     Ci      = max(minCi .* Cs,  Cs.*(1-(1.6.*VPD_l2b.^0.5./(BallBerrySlope + VPD_l2b.^0.5))));
%     gs = [];
% else
% %     %  if b > 0  Ci = Cs( 1 - 1/(m RH + b Cs/A) )
% %     % if we use Leuning 1990, Ci = Cs - (Cs - Gamma)/(m RH + b(Cs - Gamma)/A)  [see def of Gamma, above]
...
% % end
% function gs = gsFun(Cs, A, VPD_l2b, BallBerrySlope, BallBerry0)
% global sfactor
% % add in a bit just to avoid div zero. 1 ppm = 1e-6 (note since A < 0 if Cs ==0, it gives a small gs rather than maximal gs
% gs = max(BallBerry0,  (1+(BallBerrySlope * sfactor ./ VPD_l2b .^ 0.5)) .* A ./ (Cs+1e-9)  + BallBerry0 );		  
% %gs( Cs == 0 ) = would need to be max gs here;  % eliminate infinities
% gs( isnan(Cs) ) = NaN;  % max(NaN, X) = X  (MATLAB 2013b) so fix it here
% end
...
function [Ci, gs] = BallBerry(Cs, A, VPD_l2b, BallBerrySlope, BallBerry0, minCi, Ci_input)
global sfactor
BallBerrySlope = BallBerrySlope .* sfactor;
%  Cs  : CO2 at leaf surface					  
%  A   : Net assimilation in 'same units of CO2 as Cs'/m2/s
%  VPD_l2b: vapour pressure deficit from leaf to leaf boundary  [mbar or hPa]
% BallBerrySlope, BallBerry0: Medlynslope, Medlyn0 
...
function gs = gsFun(Cs, A, VPD_l2b, BallBerrySlope, BallBerry0)
% add in a bit just to avoid div zero. 1 ppm = 1e-6 (note since A < 0 if Cs ==0, it gives a small gs rather than maximal gs
gs = max(BallBerry0,  1.6.*(1+(BallBerrySlope ./ VPD_l2b .^ 0.5)) .* A ./ (Cs+1e-9)  + BallBerry0 );		  
%gs( Cs == 0 ) = would need to be max gs here;  % eliminate infinities
gs( isnan(Cs) ) = NaN;  % max(NaN, X) = X  (MATLAB 2013b) so fix it here
end

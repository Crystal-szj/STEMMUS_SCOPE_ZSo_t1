...
global SaturatedK SaturatedMC ResidualMC Coefficient_n Coefficient_Alpha porosity fieldMC RTB
SaturatedK      = ones(1,6).*para_sens.SaturatedK./(3600*24);
SaturatedMC     = ones(1,6).*para_sens.SaturatedMC;
ResidualMC      = ones(1,6).*para_sens.ResidualMC;
Coefficient_n   = ones(1,6).*para_sens.Coefficient_n;
Coefficient_Alpha = ones(1,6).*para_sens.Coefficient_Alpha;
fieldMC         = ones(1,6).*para_sens.fieldMC;
RTB             = para_sens.RTB;

ParaPlant.s2l            = para_sens.s2l;
ParaPlant.rootDensity = para_sens.rootDensity;
ParaPlant.rootRadius  = para_sens.rootRadius;
ParaPlant.beta        = para_sens.beta;
ParaPlant.rootLateralLength     = para_sens.rootLateralLength;

ParaPlant.p50Leaf        = para_sens.P50;       % parameter of plant hydraulic pathway [m] 
ParaPlant.p50Stem        = para_sens.P50;       % parameter of plant hydraulic pathway [m] 
ParaPlant.p50Root        = para_sens.P50;       % parameter of plant hydraulic pathway [m] 

ParaPlant.shapeFactorLeaf         = para_sens.ck;      % parameter  of plant hydraulic pathway, 2.95 is the default vaule in CLM
ParaPlant.shapeFactorStem         = para_sens.ck;      % parameter of plant hydraulic pathway [unitless]
ParaPlant.shapeFactorRoot         = para_sens.ck;      % parameter of plant hydraulic pathway [unitless]

ParaPlant.Krootmax          =para_sens.Krootmax;%2e-9;       % root conductivity [m s-1]
ParaPlant.Kstemmax          =para_sens.Kstemmax;%2e-8;       % stem conductivity [m s-1]
ParaPlant.Kleafmax          =para_sens.Kstemmax;%2e-7;       % maximum leaf conductance [s-1] 

g1Med                    = para_sens.g1Med;
g0Med                    = para_sens.g0Med;
